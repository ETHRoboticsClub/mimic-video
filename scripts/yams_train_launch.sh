...
COSMOS_NAME=${COSMOS_NAME:-v2w_yams_unified_iter_000007000_fused}
COSMOS_S3_PREFIX=${COSMOS_S3_PREFIX:-s3://ethrc-ml-data-916780037007/robot-learning/checkpoints/cosmos}
COSMOS_LOCAL_DIR=${COSMOS_LOCAL_DIR:-checkpoints/video_backbone}
COSMOS_LOCAL_PATH="$COSMOS_LOCAL_DIR/$COSMOS_NAME.pt"

mkdir -p "$COSMOS_LOCAL_DIR"
if [[ ! -s "$COSMOS_LOCAL_PATH" ]]; then
  echo "Downloading Cosmos checkpoint: $COSMOS_S3_PREFIX/$COSMOS_NAME.pt -> $COSMOS_LOCAL_PATH"
  aws s3 cp "$COSMOS_S3_PREFIX/$COSMOS_NAME.pt" "$COSMOS_LOCAL_PATH"
else
  echo "Using existing Cosmos checkpoint: $COSMOS_LOCAL_PATH"
fi

EXP=w2a_bi_yams_${COSMOS_NAME}_lr1.000e-04_layer20_bsz128
