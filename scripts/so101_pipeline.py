...
PIPELINE_VERSION = 7  # bumped: lowdim state/action width is inferred from LeRobot metadata
...
def read_info(dataset_root: Path, video_feature: str) -> dict[str, Any]:
    ...
    if (
        ...
        or not isinstance(state_shape, list)
        or len(action_shape) != 1
        or len(state_shape) != 1
        or not isinstance(action_shape[0], int)
        or not isinstance(state_shape[0], int)
        or action_shape[0] <= 0
        or state_shape[0] <= 0
        or action_shape != state_shape
    ):
        raise ValueError(
            f"SO101 pipeline expects matching 1D action and observation.state features in {info_path}; "
            f"got action shape {action_shape} and observation.state shape {state_shape}."
        )
    fps = info.get("fps")
...
def validate_conversion(revision_dir: Path) -> None:
    ...
    for path in action_paths:
        ...
        state_shape = root["joint_state_lowdim"].shape
        action_shape = root["joint_action_lowdim"].shape
        if (
            len(state_shape) != 2
            or len(action_shape) != 2
            or state_shape[1] <= 0
            or action_shape[1] <= 0
            or state_shape[1] != action_shape[1]
        ):
            raise ValueError(
                f"Unexpected lowdim shape in {path}: joint_state_lowdim={state_shape}, "
                f"joint_action_lowdim={action_shape}."
            )
