...
joint_state_lowdim             float32, shape (T, D)
joint_state_lowdim_timestamps  uint64,  shape (T,)
joint_action_lowdim            float32, shape (T, D)
...
The converter requires `observation.state` and `action` to be matching 1D vectors and preserves their width as `D`. The original SO101 model path uses `D=6`; the two-arm ETHRC path uses `D=14` and should train with the `so101_14` action-decoder experiment.
...
- matching low-dimensional state/action shape `(T, D)`
