...
def _log_incompatible_keys(incompat, target_name: str, max_keys: int = 20) -> None:
    """Surface missing/unexpected keys on a non-strict resume so silent re-init is visible."""
    if incompat is None:
        return
    ...
    def _trunc(keys: list[str]) -> list[str]:
        if len(keys) <= max_keys:
            return keys
        return keys[:max_keys] + [f"... (+{len(keys) - max_keys} more)"]

    log.critical(
        f"RESUME ({target_name}): non-strict load — "
        f"{len(missing)} missing key(s) re-initialized from fresh model, "
        f"{len(unexpected)} unexpected key(s) ignored from ckpt."
    )
    if missing:
        log.warning(f"RESUME ({target_name}) missing (re-initialized): {_trunc(missing)}")
    if unexpected:
        log.warning(f"RESUME ({target_name}) unexpected (ignored): {_trunc(unexpected)}")
...
class Checkpointer:
    ...
    def __init__(self, config_checkpoint: CheckpointConfig, config_job: JobConfig, callbacks: callback.CallBackGroup):
        ...
        self.keep_latest = config_checkpoint.keep_latest
        if self.keep_latest is not None and self.keep_latest < 1:
            raise ValueError("checkpoint.keep_latest must be at least 1 when set.")
    ...
    def load(
        ...
        if latest_checkpoint_file is not None:
            ...
            log.critical(
                f"AUTO-RESUME: latest_checkpoint.txt found at {checkpoint_dir}; "
                f"resuming from {latest_checkpoint_file} (model + optimizer + scheduler + grad_scaler)."
            )
        else:
            ...
            log.critical(
                f"NO RESUME: no latest_checkpoint.txt under {self.checkpoint_dir_local}; "
                "training from scratch (base DiT will be loaded from dit_path)."
            )
        ...
        if latest_checkpoint_file is not None:
            ...
            if is_fsdp:
                ...
                _log_incompatible_keys(incompat, "pipe.dit")
                # Load EMA weights.
                if model.pipe.config.ema.enabled:
                    ...
                    _log_incompatible_keys(incompat, "pipe.dit_ema")

                # Restore the attention operators.
                model.pipe.apply_cp()
            else:
                incompat = model.load_state_dict(state_dicts_to_load["model"], strict=self.strict_resume)
                _log_incompatible_keys(incompat, "model")
            torch.cuda.empty_cache()
    ...
    def _prune_old_checkpoints(self) -> None:
        ...
        folder_names = ["model", "optim", "scheduler", "trainer"]
        model_checkpoint_dir = os.path.join(self.checkpoint_dir_local, "model")
        ...
        checkpoint_files = [
            ...
            if file_name.startswith("iter_") and file_name.endswith(".pt")
        ]
        checkpoint_files.sort()
        for file_name in checkpoint_files[: -self.keep_latest]:
            for folder_name in folder_names:
                checkpoint_path = os.path.join(self.checkpoint_dir_local, folder_name, file_name)
                if os.path.exists(checkpoint_path):
                    os.remove(checkpoint_path)
                    log.info(f"Pruned old checkpoint (local): {checkpoint_path}")
