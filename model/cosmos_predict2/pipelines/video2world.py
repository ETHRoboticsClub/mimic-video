...
VIDEO_LATENT_KEY: str = "video_latent"
...
class Video2WorldPipeline(BasePipeline):
    ...
    def from_config(
        ...
        if dit_path:
            ...
            incompat = pipe.dit.load_state_dict(state_dict_dit_compatible, strict=False, assign=True)
            missing = list(getattr(incompat, "missing_keys", []) or [])
            unexpected = list(getattr(incompat, "unexpected_keys", []) or [])
            if missing or unexpected:
                log.warning(
                    f"video DiT non-strict load from {dit_path}: "
                    f"{len(missing)} missing, {len(unexpected)} unexpected."
                )
                if missing:
                    log.warning(f"  missing (re-initialized): {missing[:20]}{'... +' + str(len(missing) - 20) + ' more' if len(missing) > 20 else ''}")
                if unexpected:
                    log.warning(f"  unexpected (ignored): {unexpected[:20]}{'... +' + str(len(unexpected) - 20) + ' more' if len(unexpected) > 20 else ''}")
    ...
    def encode(self, state: torch.Tensor) -> torch.Tensor:
        ...
        full_T = self.tokenizer.get_pixel_num_frames(self.config.state_t)
        if T not in {1, 5, full_T} or (C, H, W) != (3, 480, 640):
            raise ValueError(
                f"Unexpected raw images shape {tuple(state.shape)}: encode() accepts "
                f"T in {{1, 5, full_T={full_T} (state_t={self.config.state_t})}} and "
                f"(C, H, W) = (3, 480, 640)."
            )

        encoded = self.tokenizer.encode(state) * self.sigma_data

        if encoded.shape[2] == self.config.state_t:
            return encoded

        # this part is an optimization for only encoding the obs video prefix when the rest (zeros) will be ignored.
        res = torch.zeros((B, 16, self.config.state_t, 60, 80), device=state.device, dtype=state.dtype)
    ...
    def get_mimic_data_and_condition(
        ...
        # IDM data_batch from MimicDataset has no fps; inject from pipe config.
        # Cosmos Predict2 video2world is pretrained at 10 fps (constants.py).
        data_batch["fps"] = torch.full((B, 1), int(self.config.fps))
    ...
    def get_data_and_condition(
        ...
        # Use precomputed VAE latents when available, otherwise encode online
        if VIDEO_LATENT_KEY in data_batch:
            latent_state = data_batch[VIDEO_LATENT_KEY].to(**self.tensor_kwargs)
            raw_state = latent_state  # not used in training_step (discarded with _)
        else:
    ...
    def is_image_batch(self, data_batch: dict[str, torch.Tensor]) -> bool:
        ...
        is_video_latent = VIDEO_LATENT_KEY in data_batch
        assert sum([is_image, is_video, is_video_latent]) == 1, (
            "Exactly one of the input_image_key, input_video_key, or video_latent should be present in the data_batch."
        )
        return is_image
