...
class World2ActionPipeline(BasePipeline):
    ...
    def from_config(
        ...
        if dit_path:
            ...
            if missing or unexpected:
                log.warning(
                    f"action DiT non-strict load from {dit_path}: "
    ...
    def __call__(
        ...
        # Keep an unnormalized copy of the last observed state for delta-mode
        # un-delta'ing after the network produces a denormalized prediction.
        state_last_raw = state_B_HO_O[:, -1:, :].clone() if HO > 0 else None
        ...
        sample_B_HA_A = self.normalizer.norms["action/lowdim_concat"].unnormalize(sample_B_HA_A)
        if getattr(self.config, "action_target_mode", "absolute") == "delta_from_obs_last":
            if state_last_raw is None:
                raise ValueError("action_target_mode=delta_from_obs_last requires HO > 0 (an obs reference).")
            sample_B_HA_A = sample_B_HA_A + state_last_raw  # (B, 1, A) broadcasts across action horizon
        return sample_B_HA_A
