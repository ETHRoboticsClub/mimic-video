...
class World2ActionModelConfig:
    ...
    # Probability of replacing an obs token with the learned `obs_mask_token`
    # during training. Forces the action decoder to rely on the video cross-
    # attention features (rather than memorizing proprio → action) for at least
    # `obs_dropout` of training samples. Diagnostic for the proprio-shortcut
    # failure mode: if increasing obs_dropout doesn't degrade val loss
    # meaningfully, the model isn't using the video. Originally hardcoded 0.2.
    obs_dropout: float = 0.2
...
class World2ActionModel(ImaginaireModel):
    def __init__(self, config: World2ActionModelConfig):
        ...
        self.validation_mode = getattr(config, "validation_mode", "full")
        if self.validation_mode not in {"full", "gt_video", "generated_video"}:
            raise ValueError(
                "model.config.validation_mode must be one of "
                f"'full', 'gt_video', or 'generated_video'; got {self.validation_mode!r}."
            )
    ...
    def on_train_start(self, memory_format: torch.memory_format, dataset_stats: dict, stats_id: str) -> None:
        ...
        # Stash so the SaveDatasetStats callback (fires after this) can persist
        # them next to the checkpoints; aids post-hoc denormalization at inference.
        self.dataset_stats = dataset_stats
        # Print normalization stats per data stream before they go into the
        # StaticBatchNormalizer. Helps catch bad units / unexpected ranges
        # before they silently corrupt training.
        self._log_dataset_stats(dataset_stats)
    ...
    @staticmethod
    def _log_dataset_stats(dataset_stats: dict) -> None:
        log.critical(f"Dataset normalization stats (stats_id-cached, {len(dataset_stats)} streams):")
        for key in sorted(dataset_stats):
            entry = dataset_stats[key]
            if not isinstance(entry, dict):
                continue
            mean = np.asarray(entry.get("mean"))
            std = np.asarray(entry.get("std"))
            mn = np.asarray(entry.get("min"))
            mx = np.asarray(entry.get("max"))
            with np.printoptions(precision=4, suppress=True, linewidth=160):
                log.critical(f"  {key}: shape={tuple(mean.shape)}")
                log.critical(f"    min  = {mn}")
                log.critical(f"    max  = {mx}")
                log.critical(f"    mean = {mean}")
                log.critical(f"    std  = {std}")
    ...
    def compute_loss_with_epsilon_and_t(
        ...
        vt_B_HA_A = self.pipe.denoise(
            ...
            crossattn_emb,
            video_sigma_B_1,
            obs_dropout=getattr(self.config, "obs_dropout", 0.2),
    ...
    def validation_step(self, data_batch: dict, iteration: int):
        ...
        if self.validation_mode == "gt_video":
            return output_batch, loss
        ...
        if self.validation_mode == "full":
            ...
            self.video2world_pipe.scheduler.set_timesteps(7, device=self.tensor_kwargs["device"])
        ...
        assert T in (1, 5)
        num_pixel_frames = self.video2world_pipe.tokenizer.get_pixel_num_frames(self.config.video_pipe_config.state_t)
        vid_input = torch.zeros((B, C, num_pixel_frames, H, W), device=input_vid.device, dtype=input_vid.dtype)
