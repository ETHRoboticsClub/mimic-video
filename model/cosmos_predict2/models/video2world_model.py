...
from cosmos_predict2.pipelines.video2world import VIDEO_LATENT_KEY, Video2WorldPipeline
...
class Predict2Video2WorldModel(ImaginaireModel):
    ...
    def add_lora_to_model(
        ...
        log.info(
            f"Adding LoRA adapters: rank={lora_rank}, alpha={lora_alpha}, dropout={lora_dropout}, "
            f"targets={target_modules_list}"
        )
    ...
    def _update_train_stats(self, data_batch: dict[str, torch.Tensor]) -> None:
        ...
        if not is_image and input_key not in data_batch:
            input_key = VIDEO_LATENT_KEY
    ...
    def compute_loss_with_epsilon_and_sigma(
        ...
        with torch.no_grad():
            # correction=0 (population variance) is well-defined for batch_size=1
            # validation steps; default Bessel correction (n-1) divides by zero there.
            var_x0 = x0_B_C_T_H_W.float().var(dim=0, correction=0).mean()
            mean_x0 = x0_B_C_T_H_W.float().mean()
            var_eps = epsilon_B_C_T_H_W.float().var(dim=0, correction=0).mean()
            sigma_B_T = rearrange(sigma_B_T.float(), "b t -> b 1 t 1 1")
            var_xt = (xt_B_C_T_H_W.float() / (1.0 + sigma_B_T)).var(dim=0, correction=0).mean()
            var_pred = model_pred.x0.float().var(dim=0, correction=0).mean()
