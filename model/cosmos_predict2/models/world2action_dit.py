...
class World2ActionDIT(nn.Module):
    ...
    def prepare_embedded_sequence(
        ...
        *,
        ...
        if state_B_HO_O.shape[-1] != self.in_channels:
            raise ValueError(f"Expected obs lowdim width {self.in_channels}, got {state_B_HO_O.shape}.")
        if xt_B_HA_A.shape[-1] != self.out_channels:
            raise ValueError(f"Expected action width {self.out_channels}, got {xt_B_HA_A.shape}.")

        obs_emb = self.obs_embedder(state_B_HO_O)
        action_emb = self.action_embedder(xt_B_HA_A)
        seq_len = obs_emb.shape[1] + action_emb.shape[1]
        if seq_len > self.max_horizon:
            raise ValueError(
                f"Action decoder sequence length {seq_len} exceeds max_horizon={self.max_horizon}; "
                f"obs={obs_emb.shape[1]}, actions={action_emb.shape[1]}."
            )
        ...
        return torch.cat((obs_emb, action_emb), dim=1) + self.pos_embedding[:, :seq_len]
