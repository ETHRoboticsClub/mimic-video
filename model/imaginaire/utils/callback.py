...
class EMAModelCallback(Callback):
    ...
    def on_train_start(self, model: ImaginaireModel, iteration: int = 0) -> None:
        ...
        if model.config.pipe_config.ema.enabled:
            assert hasattr(model.pipe, "dit_ema_worker"), "EMA should be initialized from ImaginaireModel"
        else:
            assert not hasattr(model, "dit_ema_worker"), "There should be no EMA initialized."
    ...
    def on_training_step_end(
        ...
        if model.config.pipe_config.ema.enabled:
            model.pipe.dit_ema_worker.update_average(model, iteration)
...
def _to_wandb_config(value: Any) -> Any:
    ...
    if isinstance(value, omegaconf.DictConfig | omegaconf.ListConfig):
        value = omegaconf.OmegaConf.to_container(value, resolve=True, throw_on_missing=False)
        return _to_wandb_config(value)
    if attrs.has(value.__class__):
        return {field.name: _to_wandb_config(getattr(value, field.name)) for field in attrs.fields(value.__class__)}
...
class WandBCallback(Callback):
    """Initializes W&B on rank 0 and logs scalar training and validation losses."""
    ...
    def on_train_start(self, model: ImaginaireModel, iteration: int = 0) -> None:
        ...
        wandb.init(
            ...
            dir=self.config.job.path_local,
            config=_to_wandb_config(self.config),
        )
    ...
    def on_training_step_end(
        ...
        if self.enabled and wandb.run is not None:
            wandb.log({"train/loss": loss.detach().item()}, step=iteration)
    ...
    def on_before_optimizer_step(
        ...
        if not (self.enabled and wandb.run is not None):
            return
        param_groups = optimizer.param_groups
        if len(param_groups) == 1:
            payload: dict[str, float] = {"train/lr": float(param_groups[0]["lr"])}
        else:
            payload = {
                f"train/lr/group_{idx}": float(group["lr"]) for idx, group in enumerate(param_groups)
            }
    ...
    def on_validation_end(self, model: ImaginaireModel, iteration: int = 0) -> None:
        ...
        if self._val_loss_count > 0:
            wandb.log(
                {"val/loss": self._val_loss_sum / self._val_loss_count},
                step=iteration,
            )
